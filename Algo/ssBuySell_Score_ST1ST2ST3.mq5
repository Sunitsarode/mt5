...
input string exitbytrend = "any"; // st1 | st2 | st3 | any
...
int OnInit()
{
   ...
   {
      PrintFormat("Failed to create ST2 handle (%s). err=%d", EnumToString(PERIOD_M5), GetLastError());
      return(INIT_FAILED);
   }
   ...
   {
      PrintFormat("Failed to create ST3 handle (%s). err=%d", EnumToString(PERIOD_H1), GetLastError());
      return(INIT_FAILED);
   }
   ...
   PrintFormat("Trend filters: ST1=%s ST2=%s ST3=%s exitbytrend=%s",
               EnumToString(PERIOD_CURRENT), EnumToString(PERIOD_M5), EnumToString(PERIOD_H1), exitbytrend);
