//+------------------------------------------------------------------+
//|                         ScoreSell with fixed target handle all position with magic no Strategy (EA)                  |
//| Uses external indicators: Sumit_RSI_Score_Indicator + supertrend |
//| Sell when score is high and configured previous candle level breaks up
//| chk_last_candle_breaks = 0/H/L/O/C
//| chk_last_candle_type = 0/G/R
//+------------------------------------------------------------------+
...
input int EntryScoreThreshold = 70; // Score must be >= threshold
...
void CheckTargets(const bool hedging, const double current_price)
{
   ...
   for(int i = 0; i < total; i++)
   {
      ...
      else
      {
         ...
         // For sells, track the lowest favorable price after trailing activation.
...
int OnInit()
{
   ...
   Print("Strategy uses external score + H1 SuperTrend bearish filter.");
...
void OnTick()
{
   ...
   // Sell gating condition: SuperTrend on H1 must be bearish.
   ...
   if(openCount == 0)
   {
      ...
      if(!IsLastCandleBreakConditionMet(ask))
         return;

      if(last_entry_price_ref > 0.0)
      {
         if(MathAbs(ask - last_entry_price_ref) < CalculateStepDistance(last_entry_price_ref))
            return;
      }

      TryOpenEntry(bid, LotSize, hedging);
      return;
   }
   ...
   if(last_entry_price_open > 0.0 && ask >= (last_entry_price_open + CalculateStepDistance(last_entry_price_open)))
   {
      double nextLot = last_entry_lot;
      if(nextLot <= 0.0)
         nextLot = LotSize;
      nextLot += LotStep;
      TryOpenEntry(bid, nextLot, hedging);
   }
}
